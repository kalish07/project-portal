...
const getDocumentState = (url) => {
  ...
  return "Approved"; // You can adjust this logic if needed
};
  ...
  return (
    <div className="border rounded-lg p-4 hover:shadow-md transition-shadow duration-200">
      <div className="flex items-start space-x-3">
        <div className={`p-2 rounded-lg ${stateDetails.badgeColor}`}>
          {stateDetails.icon}
        </div>
            ...
            const docs = [
              {
                ...
                type: "Abstract",
                state: getDocumentState(project.abstract_url),
                version: 1,
              },
              {
                id: `${project.id}-report`,
                type: "Report",
                state: getDocumentState(project.report_pdf_url),
                version: 1,
              },
              {
                id: `${project.id}-ppt`,
                type: "Presentation",
                state: getDocumentState(project.ppt_url),
                version: 1,
              },
            ];

            return (
              <div key={project.id} className="border border-gray-100 rounded-lg p-5 bg-gray-50/50">
